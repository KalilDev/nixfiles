{config, lib, pkgs, ...}: {
  ...
  options.sway.home.packages = lib.mkOption {
    type = with lib.types; listOf package;
    default = [];
    description = "Extra packages for sway config";
  };
  config =
  let
    super = "Mod4";
  in {
    sway.home.packages = with pkgs; (config.flameshot.home.packages or []) ++ (config.waybar.home.packages or []) ++ [
      swaybg
      playerctl
      brightnessctl
    ];
